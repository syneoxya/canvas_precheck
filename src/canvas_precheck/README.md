...
By default, posting is disabled.
...
Feedback will be posted as a submission comment in Canvas.

⸻

LangChain Usage

LangChain is used only for the LLM reviewer:
	•	Deterministic logic is handled in Python
	•	LLM input is constrained
	•	Output follows a strict JSON schema
	•	Evidence keys are required to avoid hallucination
...
The system works fully without LangChain if LLM is disabled.

⸻

Design Rationale
	•	Multi-agent pipeline → modular, testable, extensible
	•	Config-driven assignments → scalable across courses
	•	Deterministic checks first → reliability and transparency
	•	LLM as an optional reviewer → augmentation, not authority
	•	Local secrets for development → simplicity during prototyping
...
Limitations & Future Work
	...
	•	Text-entry Canvas submissions not handled
	•	No persistent database
	•	No production deployment (OAuth/LTI)

These were intentionally scoped out for a class prototype.
...
This project demonstrates:
	...
	•	Config-driven scalability
	•	Responsible use of LLMs
	•	Clear separation of concerns

It is suitable as a technical prototype for automated assignment pre-checks in educational settings.

⸻

If you want, I can:
	•	tailor this README exactly to your professor’s rubric
	•	add an Architecture Diagram section
	•	or rewrite it in a more academic / paper-style tone